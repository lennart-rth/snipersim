...
UInt32 LoadSliceBiClassifier::ip_to_tagoff(const IntPtr ip) const {
  return ip / m_entries;
}

LoadSliceBiClassifier::Way::Way(const UInt64 entries)
...
void LoadSliceBiClassifier::update(DynamicMicroOp &microOp, const RegisterDependencies& reg_dep, const uint64_t lowestValidSequenceNumber)
{
